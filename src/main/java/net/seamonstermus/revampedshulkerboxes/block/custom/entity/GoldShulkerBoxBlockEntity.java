...
public class GoldShulkerBoxBlockEntity extends LootableContainerBlockEntity implements SidedInventory {
	...
	private GoldShulkerBoxBlockEntity.AnimationStage animationStage = GoldShulkerBoxBlockEntity.AnimationStage.CLOSED;
	private float animationProgress;
	private float prevAnimationProgress;
	@Nullable
	private final DyeColor cachedColor;

	public GoldShulkerBoxBlockEntity(@Nullable DyeColor color, BlockPos pos, BlockState state) {
		super(BlockEntityType.SHULKER_BOX, pos, state);
		this.cachedColor = color;
	}

	public GoldShulkerBoxBlockEntity(BlockPos pos, BlockState state) {
		super(BlockEntityType.SHULKER_BOX, pos, state);
		this.cachedColor = ShulkerBoxBlock.getColor(state.getBlock());
	}

	public static void tick(World world, BlockPos pos, BlockState state, GoldShulkerBoxBlockEntity blockEntity) {
		...
		switch (this.animationStage) {
			...
			case OPENING:
				...
				if (this.animationProgress >= 1.0F) {
					this.animationStage = GoldShulkerBoxBlockEntity.AnimationStage.OPENED;
			...
			case CLOSING:
				...
				if (this.animationProgress <= 0.0F) {
					this.animationStage = GoldShulkerBoxBlockEntity.AnimationStage.CLOSED;
					this.animationProgress = 0.0F;
					updateNeighborStates(world, pos, state);
				}
				break;
	...
	public GoldShulkerBoxBlockEntity.AnimationStage getAnimationStage() {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (entity.getPistonBehavior() != PistonBehavior.IGNORE) {
						entity.move(
							...
							new Vec3d(
								(box.getXLength() + 0.01) * direction.getOffsetX(),
								(box.getYLength() + 0.01) * direction.getOffsetY(),
								(box.getZLength() + 0.01) * direction.getOffsetZ()
							)
						);
					}
				}
	...
	public boolean onSyncedBlockEvent(int type, int data) {
		if (type == 1) {
			...
			if (data == 0) {
				this.animationStage = GoldShulkerBoxBlockEntity.AnimationStage.CLOSING;
				updateNeighborStates(this.getWorld(), this.pos, this.getCachedState());
			}

			if (data == 1) {
				this.animationStage = GoldShulkerBoxBlockEntity.AnimationStage.OPENING;
				updateNeighborStates(this.getWorld(), this.pos, this.getCachedState());
			}
	...
	public boolean suffocates() {
		return this.animationStage == GoldShulkerBoxBlockEntity.AnimationStage.CLOSED;
	}
